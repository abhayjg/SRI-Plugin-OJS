 ...
 * form via AJAX (mirrors OJS's DOI settings form behavior).
 */
(function($) {
	'use strict';
	...
	$.pkp.plugins.pubIds.sri.js.SriSettingsFormHandler =
		function($form, options) {
			// OJS's parent() helper resolves constructors through Function.caller,
			// which is unavailable when this strict-mode constructor calls it.
			...
			// Account status is fetched through the OJS server so the SRI API key
			// never reaches browser JavaScript or a third-party origin.
		...
		.loadAccountStatus_ = function() {
			...
			request.done(function(jsonData) {
				...
				if (jsonData && jsonData.status === true &&
					...
					// The partial is produced by the plugin and escapes all API data.
